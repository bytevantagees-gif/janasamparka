...
Revision ID: eea1551a83eb
Revises: c0fc432d3f05
Create Date: 2025-10-30 06:19:54.517608
...
revision = 'eea1551a83eb'
down_revision = 'c0fc432d3f05'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # Simplified: Only add profile_photo column, skip PostGIS table drops
...
def downgrade() -> None:
    ...
    op.alter_column('department_routing', 'reason',
               existing_type=sa.Enum('incorrect_department', 'better_suited', 'specialized_team', 'jurisdiction', 'other', name='routingreason', native_enum=False),
               type_=sa.VARCHAR(length=50),
               existing_nullable=False)
    op.create_foreign_key('complaints_parent_complaint_id_fkey', 'complaints', 'complaints', ['parent_complaint_id'], ['id'])
    op.create_index('ix_complaints_priority_score', 'complaints', ['priority_score'], unique=False)
    op.create_index('ix_complaints_parent_complaint_id', 'complaints', ['parent_complaint_id'], unique=False)
    op.create_index('ix_complaints_is_emergency', 'complaints', ['is_emergency'], unique=False)
    op.create_index('ix_complaints_is_duplicate', 'complaints', ['is_duplicate'], unique=False)
    op.alter_column('complaint_escalations', 'reason',
               existing_type=sa.Enum('incorrectly_closed', 'wrongly_routed', 'no_progress_update', 'delayed_resolution', 'poor_quality_work', 'unresponsive', 'other', name='escalationreason', native_enum=False),
               type_=sa.VARCHAR(length=50),
               existing_nullable=False)
    op.alter_column('case_notes', 'note_type',
               existing_type=sa.Enum('general', 'status_update', 'department_routing', 'escalation', 'resolution', 'work_update', name='notetype', native_enum=False),
               type_=sa.VARCHAR(length=50),
               existing_nullable=False)
