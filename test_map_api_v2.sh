...
OTP_RESPONSE=$(curl -s -X POST http://localhost:8000/api/auth/request-otp \
...
echo "2. Verifying OTP=$OTP and getting token..."
TOKEN_RESPONSE=$(curl -s -X POST http://localhost:8000/api/auth/verify-otp \
  -H "Content-Type: application/json" \
  -d "{\"phone\": \"+918242226666\", \"otp\": \"$OTP\"}")
...
TOTAL=$(echo "$COMPLAINTS_RESPONSE" | jq '.total // 0')
...
if [ "$WITH_COORDS" == "0" ]; then
  echo "  ❌ NO COMPLAINTS WITH COORDINATES FOUND!"
fi
...
echo "$COMPLAINTS_RESPONSE" | jq -r '.complaints[]? | select(.lat == null or .lng == null) | "  - \(.title)"' | head -5
